...
function getWarehousesByID(id) {
  ...
  let filteredItems = warehousesArray.filter(
...
function patchWarehouse(body) {
  const warehousesArray = listWarehouses()
  let newWareArray = warehousesArray
    .filter((warehouses) => warehouses.id === body.id)
    .pop()

  if (body.name) {
    const inventoriesArray = listInventories()

    let newInvArray = inventoriesArray
      .filter((inventory) => inventory.warehouseID === newWareArray.id)
      .map((filteredItem) => {
        return { ...filteredItem, warehouseName: body.name }
      })

    let inventoryUpdated = inventoriesArray.filter(
      (inventory) => inventory.warehouseID !== newWareArray.id
    )
